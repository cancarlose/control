...
class Produtos extends MY_Controller
{
    ...
    public function editar()
    {
        ...
        if ($this->form_validation->run('produtos') == false) {
            ...
            $precoCompraNovo = str_replace(',', '', $this->input->post('precoCompra'));
            $precoVendaNovo = str_replace(',', '', $this->input->post('precoVenda'));
    
            // produto atual do banco de dados para comparar preços
            $produtoAtual = $this->produtos_model->getById($this->input->post('idProdutos'));
            $precoCompraAntigo = $produtoAtual->precoCompra;
            $estoqueAtual = $produtoAtual->estoque;
    
            // diferença de preço
            $diferencaPreco = $precoCompraNovo - $precoCompraAntigo;
    
            // variação total no estoque
            $variacaoPrecoEstoque = $diferencaPreco * $estoqueAtual;
    
            $data = [
                'codDeBarra' => set_value('codDeBarra'),
                'descricao' => $this->input->post('descricao'),
                'unidade' => $this->input->post('unidade'),
                'precoCompra' => $precoCompraNovo,
            ...
            if ($this->produtos_model->edit('produtos', $data, 'idProdutos', $this->input->post('idProdutos')) == true) {
                $this->session->set_flashdata('success', 'Produto editado com sucesso! Variação de preço no estoque: R$' . number_format($variacaoPrecoEstoque, 2));
