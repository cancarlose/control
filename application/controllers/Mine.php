...
class Mine extends CI_Controller
{
    ...
    public function gerarTokenResetarSenha()
    {
        if (! $cliente = $this->check_credentials($this->input->post('email'))) {
            ...
            $this->session->set_userdata($session_mine_data);
            log_info('Técnico solicitou alteração de senha. Porém falhou ao realizar solicitação!');
            ...
            if ($this->resetSenhas_model->add('resets_de_senha', $data) == true) {
                ...
                $this->session->set_userdata($session_mine_data);
                log_info('Técnico solicitou alteração de senha.');
    ...
    public function visualizarOs($id = null)
    {
        ...
        if ($data['result']->idClientes != $this->session->userdata('cliente_id')) {
            $this->session->set_flashdata('error', 'Esta OS não pertence ao Técnico logado.');
    ...
    {
        print_r(json_encode(['code' => 4001, 'error' => 'Erro interno', 'errorDescription' => 'Cobrança não pode ser gerada pelo lado do Técnico']));
    ...
    public function adicionarOs()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (is_numeric($id = $this->Conecte_model->add('os', $data, true))) {
                ...
                $this->enviarOsPorEmail($idOs, $remetentes, 'Nova Ordem de Serviço #' . $idOs . ' - Criada pelo Técnico');
